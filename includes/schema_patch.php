...
function ensureAppSchema(PDO $pdo): void
{
    ...
    try {
        ...
        $bridgeCol = $pdo->query("SHOW COLUMNS FROM shops LIKE 'print_bridge_token'")->fetch();
        if (!$bridgeCol) {
            $pdo->exec(
                'ALTER TABLE shops
            ...
            try {
                $pdo->exec('ALTER TABLE shops ADD UNIQUE KEY uq_print_bridge_token (print_bridge_token)');
            } catch (Throwable $e) {
                // index may already exist
            }
        }
